...
file = open("suppose_$(split(init["p4p_file"], ".")[1]).txt", "w")
write(file, "h\tk\tl\tv_n\tv_f\tff\t2θ\tϕ\tω_0\tω_m\tω_p\tω_mf\tω_p\n")

for hkl in signperm(20, 10, 4)
    hkl[1] < 0 && continue
    result = process_hkl(hkl)
